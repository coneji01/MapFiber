...
app.post('/api/naps/:id/splitters', (req, res) => {
  ...
  // Insert into manga_splitters (FK target)
  db.prepare('INSERT OR IGNORE INTO manga_splitters (id, manga_id, name, splitter_type_id, ports_count) VALUES (?, ?, ?, ?, ?)')
    .run(splitterId, mangaId, name || 'NAP Splitter', splitter_type_id, numPorts);
  
  // Create manga_fibers para el splitter NAP con splitter_id asignado
...
app.post('/api/splices', (req, res) => {
  ...
  if (cableConnId && mangaFiberId) {
    ...
    if (cablePt) {
      ...
      // Fallback: si es splice OUTPUT, la potencia viene del splitter (manga_fiber)
      if (!fc && mangaFiberId) {
      ...
      // Fallback: trazar hacia atras por fusiones iterativamente
      if (!fc) {
